...
function Random() {
  ...
  const [questionResponses, setQuestionResponses] = useState({}); // État pour stocker les réponses générées avec les questions
  ...
  const generateRandomQuestion = async () => {
    setLoading(true);
    setAnswered(false); // Réinitialiser l'état de la réponse précédente

    const response = await fetch(`${API_BASE}/generate-random-question`);
  ...
  const handlePreviousQuestionClick = (prevQuestion) => {
    ...
    if (questionResponses[prevQuestion]) {
      setFeedback(questionResponses[prevQuestion]); // Afficher la réponse générée pour la question sélectionnée
    }
  };
  ...
  return (
    <div className="d-flex">
      ...
      <div className="main-content p-4" style={{ flex: 1 }}>
        ...
        <div className="mb-3">
          <button className="btn btn-primary w-100 mt-2" onClick={generateRandomQuestion}>
            Générer Question
          </button>
          ...
          {/* Div pour afficher la réponse générée */}
          {feedback && answered && (
            <div className="ai-feedback mt-3" dangerouslySetInnerHTML={{ __html: formatFeedback(feedback) }} />
          )}

          {/* Div pour afficher la réponse générée lorsque la réponse n'a pas encore été donnée */}
          {feedback && !answered && (
            <div className="ai-feedback mt-3" dangerouslySetInnerHTML={{ __html: formatFeedback(feedback) }} />
          )}

          {feedback && (
            <button className="btn btn-secondary w-100 mt-2" onClick={generateRandomQuestion}>
              Nouvelle Question
            </button>
          )}
        </div>
