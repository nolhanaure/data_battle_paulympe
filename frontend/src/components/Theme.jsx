...
function Theme() {
  ...
  const [questionReponses, setQuestionReponses] = useState({}); // Nouveau état pour stocker les réponses générées par l'IA pour chaque question

  const API_BASE = "http://127.0.0.1:8000";

  async function genQuestion(str) {
    setCategory(str);
    setLoading(true);
    
    setAnswered(false); // Réinitialiser l'état de réponse lors de la génération d'une nouvelle question

    const response = await fetch(`${API_BASE}/generate-question?category=${encodeURIComponent(str)}`);
  ...
  const analyzeAnswer = async () => {
    if (!userAnswer.trim()) {
      // Si l'utilisateur n'a pas encore répondu, afficher un message d'erreur
    ...
    // Stocker la réponse générée pour la question
  ...
  const handlePreviousQuestionClick = (prevQuestion) => {
    ...
    // Récupérer la réponse générée pour la question précédente
  ...
  const handleNewQuestionClick = () => {
    ...
    setUserAnswer('');
    setFeedback('');
    setAnswered(false); // Réinitialiser l'état `answered` pour permettre une nouvelle question
  };
  ...
  return (
    <div className="d-flex">
      {/* Panneau latéral gauche pour les anciennes questions */}
      <div className="sidebar p-3" style={{ width: '250px', backgroundColor: 'transparent', borderRight: '2px solid #ddd' }}>
        <h5>Anciennes Questions :</h5>
        <ul>
          {previousQuestions.map((q, index) => (
            <li key={index} className="cursor-pointer" onClick={() => handlePreviousQuestionClick(q)}>
              {q}
            </li>
          ))}
        </ul>
      ...
      <div className="main-content p-4" style={{ flex: 1 }}>
        ...
        <div className="mb-3">
          <input
            ...
            placeholder="Catégorie (ex. inventeur)..."
            onKeyDown={(e) => e.key === "Enter" && generateQuestion()}
          />
          <button className="btn btn-primary w-100 mt-2" onClick={generateQuestion}>
            Générer Question
          </button>
          <div className="row mt-3">
            <button className="scd btn" onClick={() => genQuestion('Brevet')}>Brevet</button>
            <button className="scd btn" onClick={() => genQuestion('Inventor')}>Inventeur</button>
            <button className="scd btn" onClick={() => genQuestion('Law')}>Loi</button>
            <button className="scd btn" onClick={() => genQuestion('Property')}>Propriété</button>
          </div>
          <div className="row mt-3">
            <button className="scd btn" onClick={() => genQuestion('Brand')}>Marque</button>
            <button className="scd btn" onClick={() => genQuestion('Innovation')}>Innovation</button>
            <button className="scd btn" onClick={() => genQuestion('Plagiat')}>Plagiat</button>
            <button className="scd btn" onClick={() => genQuestion('Contrefaçon')}>Contrefaçon</button>
          </div>
        </div>
        ...
        {question && (
          <>
            <div className="alert mt-3">{question}</div>
            <textarea
              rows="4"
              className="form-control"
              value={userAnswer}
